				   ...
				   bool ingress)
{
	...
	case FLOW_ACTION_POLICE:
		return lan966x_police_port_add(port, &f->rule->action, act,
					       __c_ua(f->cookie), ingress,
					       f->common.extack);
	case FLOW_ACTION_MIRRED:
		return lan966x_mirror_port_add(port, act, __c_ua(f->cookie),
					       ingress, f->common.extack);
	case FLOW_ACTION_GOTO:
		return lan966x_goto_port_add(port, f->common.chain_index,
					     act->chain_index, __c_ua(f->cookie),
					     f->common.extack);
	default:
		NL_SET_ERR_MSG_MOD(f->common.extack,
				   "Unsupported action");
				   ...
				   bool ingress)
{
	if (f->cookie == port->tc.police_id) {
		return lan966x_police_port_del(port, __c_ua(f->cookie),
					       f->common.extack);
		...
		return lan966x_goto_port_del(port, __c_ua(f->cookie), f->common.extack);
	}
