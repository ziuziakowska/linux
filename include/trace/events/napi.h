...
TRACE_EVENT(napi_poll,
	...
	TP_fast_assign(
		__assign_ptr(napi, napi);
		__assign_str(dev_name);
		__entry->work = work;
		__entry->budget = budget;
	),

	TP_printk("napi poll on napi struct " TRACE_CAP_FMT " for device %s work %d budget %d",
		  __get_cap(napi), __get_str(dev_name),
